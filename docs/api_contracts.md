...
Server-to-server webhook callback. Performs signature checks on the raw string request body and processes order confirmations/failures idempotently.

**Headers:**
- `x-razorpay-signature`: HMAC-SHA256 signature generated by Razorpay.

**Request Body (Raw Success Payload):**
```json
{
...
**Request Body (Raw Failure Payload):**
```json
{
...
```
*(Returns `200 OK` immediately if the order was already marked as PAID to prevent double-deducting inventory. On `payment.failed`, marks the order as `PAYMENT_FAILED` and reverts session `authorizationState` to `NONE`).*

---

## 6. Demo Sandbox Scenario Preparation

### `POST /api/demo/scenario`
Pre-configures the catalog database records, policy autonomy level, and budgets to execute specific live-demonstration trust scenarios.
...
```
*Supported Scenarios:*
- `1` (Perfect Match): Sets policy autonomy to Level 3 (Autonomous), SwiftRun Blue Trainer is marked in-stock.
- `2` (Constraint Negotiation): Sets policy autonomy to Level 2 (Prepare), SwiftRun Blue Trainer is set to out-of-stock (stock=0), enabling alternatives relaxation flow.
- `3` (Safety Policy Block): Sets policy autonomy to Level 3 (Autonomous), maximum policy budget set to ₹2,000, TrailBlazer Premium Runner priced at ₹2,499.
...
```json
{
  ...
  "message": "Demo Scenario 2 catalog configuration seeded successfully."
}
```
